/************************************************************************************
 * " Assignment #3: PP3 (Huffman Code Generator) "                                  *
...
void HuffmanTree::decompress_tree(std::string serializedTree)                                               //reconstruct(deserialize) the serializedTree string 
{
    ...
    while(x < serializedTree.length()) {                                                                    //while loop indexes as long 'x' is LESS than the length of serializedTree string
...
{
   if(!hq.empty()) {                //if heap queue is not empty
...
{
   if(root->left!=nullptr) { deconstruct_tree(root->left); }        //while left node is not a nullptr, recursively proceed to next node
   else if(root->left!=nullptr) { deconstruct_tree(root->right); }  //while right node is not a nullptr, recursively proceed to next node
   if(root) { delete root; }                                        //when root is located, delete root
}//end deconstruct_tree

void HuffmanTree::deallocate_heap()                                 //memory deallocating of heap queue method
{
    while (!hq.empty()) {                                           //while heap queue is not empty,
