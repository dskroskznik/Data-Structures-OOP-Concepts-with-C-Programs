...
void NotationConverter::loadDeque(std::string newStr){                                                                          //Deque build loads string in elements by character
    while(!LD.empty()){                                                                                                         //deque is not empty?    
    ...
    {
        if(x == '(' || x == ')' || x == ' '){                                                                                   //passes along deque elements if any 
            continue;                                                                                                           //...spaces or parenthesesare found
        }                                                                                                                               
        else if((x >= 'a' && x <= 'z') || (x >= 'A' && x <= 'Z') || (x == '/') || (x == '*') || (x == '+') || (x == '-')) {     //else...if 'x' is alphabetic letter or function operator characters then,    
...
std::string NotationConverter::postfixToInfix(std::string inStr){                                   //postfixToInfix notation conversion
    loadDeque(inStr);                                                                               //call loadDeque function to load string data in
    while(LD.size() > 1){                                                                           //while the deque holds size > 1 element
        if (LD.front() != "/" && LD.front() != "*" && LD.front() != "+" && LD.front() != "-"){      //if any operator char is NOT in front of deque then,
            ...
            if (LD.front() != "/" && LD.front() != "*" && LD.front() != "+" && LD.front() != "-"){  //...if any operator char is NOT in front of deque again then,
...
std::string NotationConverter::postfixToPrefix(std::string inStr){                                  //postfixToInfix notation conversion
    return infixToPrefix(postfixToInfix(inStr));                                                    //calls appropriate converter function to fit notation
}//end postfixToPrefix

std::string NotationConverter::infixToPostfix(std::string inStr){                                   //postfixToInfix notation conversion
    return prefixToPostfix(infixToPrefix(inStr));                                                   //calls appropriate converter function to fit notation
}//end infixToPostfix

std::string NotationConverter::infixToPrefix(std::string inStr){                                    //postfixToInfix notation conversion
    loadDeque(inStr);                                                                               //call loadDeque function to load string data in
    while(LD.size() > 1){                                                                           //while the deque holds size > 1 element
        if (LD.front() != "/" && LD.front() != "*" && LD.front() != "+" && LD.front() != "-"){      //if any operator char is NOT in front of deque then,
            std::string temp = LD.front();                                                          //...put front element into a temp string value
            ...
            if (LD.front().compare("/") == 0 || LD.front().compare("*") == 0 ||                     //if follow front element compares similar to any operator
             LD.front().compare("+") == 0 || LD.front().compare("-") == 0){                             //then,
...
std::string NotationConverter::prefixToInfix(std::string inStr){                                    //prefixToInfix notation conversion
...
std::string NotationConverter::prefixToPostfix(std::string inStr){
    loadDeque(inStr);                                                                               //call loadDeque function to load string data in
    while(LD.size() > 1){                                                                           //while the deque holds size > 1 element    
        if (LD.front() == "/" || LD.front() == "*" || LD.front() == "+" || LD.front() == "-"){      //if any operator char IS in front of deque then,
            std::string temp = LD.front();                                                          //...put front element into a temp string value
            LD.removeFront();                                                                       //...and remove element from the deque, interation continuing
            if (LD.front() != "/" && LD.front() != "*" && LD.front() != "+" && LD.front() != "-"){  //...if any operator char is NOT in front of deque again then,
