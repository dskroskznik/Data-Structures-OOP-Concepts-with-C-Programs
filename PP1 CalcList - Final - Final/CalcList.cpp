/************************************************************************************
 * " Assignment #1: Pair Programming 1 (Calculator) "                               
 *  by Gabriel Adelemoni & Dylan Skroskznik                                         
 *  
 * CalcList source file to implement Doubly Linked List class functions.
...
CalcList::CalcList() {//constructor function
...
CalcList::~CalcList() {//destructor function
    while(!empty()) removeLastOperation();  //undo all operations until sentinels
    delete head;                            //delete sentinels
    delete base;                            
    delete trail;                           
}//end ~CalcList

bool CalcList::empty() const {//boolean function if list is empty
    return (base->next == trail);   //have base sentinel point to end trail node
}//end empty

double CalcList::total() const {//keep element number total at final node before trail
    return trail->prev->element;    //assigns element to trails previous node
}//end total

void CalcList::addToBack(const double e) {//insert new node for point to end of list
...
void CalcList::newOperation(const FUNCTIONS func, const double element) {//arithemtic operations provided in each node
    if(func == ADDITION) {                                                  //Addition Function:                                                 
    ...
    if(func == SUBTRACTION) {                                               //Subtraction Function:
        addToBack(total() - element);                                       //subtracts element to total func to back node
        trail->prev->prev->Operation = "-";                                 //declares operation character to variable    
    }

    if(func == MULTIPLICATION) {                                            //Multiplication Function:
        addToBack(total() * element);                                       //multiplies element to total func to back node
        trail->prev->prev->Operation = "*";                                 //declares operation character to variable
    }

    if(func == DIVISION) {                                                  //Division Function:    
        if(element == 0){                                                   //checks if element value is '0'
...
void CalcList::removeLastOperation() {//undo last operation function
    if (empty()) {                                                                              //check for empty linked list
...
std::string CalcList::toString(unsigned short precision) const {//string functions, outputs calculator operations list with implemented precision                                                                                              
    ...
    while (numOfOperations > 0) {                                                                       //while operations done greater than '0'
